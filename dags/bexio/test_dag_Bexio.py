...
default_args = {
    'owner': 'test_owner',
    'start_date': '2023-09-29',  # Adjust the start date as needed
    'retries': 1,
}

dag = DAG(
    'Bexio_ch_invoice_and_line_items_old',
    default_args=default_args,
    schedule_interval=None,  # You can set the schedule interval here
    catchup=False,
)
...
def fetch_data(ti):
    ...
    url = 'https://api.bexio.com/2.0/kb_invoice?offset=12000&limit=500' 
...
def transform_and_insert_to_bq(**kwargs):
    ...
    sql_query = f"delete FROM `bi-data-replication-gcs.bexio_ch.invoices`  where bexio_id IN ({ids_to_delete});"
    ...
    df['percentage'] = df['taxs'].apply(lambda tax_list: tax_list[0]['percentage'] if tax_list else None)
    print(df['percentage'])
