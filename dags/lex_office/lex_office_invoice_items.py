...
dag = DAG(
    'Lexoffice_invoice_items',
    default_args=default_args,
    description='Lex office invoice line items',
...
def fetch_payments(args):
    time.sleep(3)  
    id, headers = args
    url = f"https://api.lexoffice.io/v1/invoices/{id}"
    ...
    for result in results:
        ...
        if data:
                ...
                for line_item in data['lineItems']:
                    ...
                    if 'discountPercentage' in line_item:
                        discountPercentage = line_item['discountPercentage']
                    else:
...
# Continue only if ids_to_delete is not empty
    ...
    print(df.columns)
    columns_to_convert = [ 'netAmount', 'grossAmount', 'taxRatePercentage', 'discountPercentage',
            'lineItemAmount',    'totalNetAmount',  'totalGrossAmount',
            'totalTaxAmount' ]
    df[columns_to_convert] = df[columns_to_convert].apply(pd.to_numeric, errors='coerce')
   
    desired_order =  ['invoice_id', 'organisation_id', 'countrycode',  'productid', 'type',
            'name',  'description', 'quantity', 'unitName',   'netAmount',
            'grossAmount', 'taxRatePercentage', 'discountPercentage',
            'lineItemAmount', 'currency', 'totalNetAmount',  'totalGrossAmount',
            'totalTaxAmount', 'updated_at']
...
execute_query = BigQueryExecuteQueryOperator(
    ...
    sql=sql_query,
    destination_dataset_table='lexoffice.temp_invoices',  # Specify your dataset and a temporary table to store the results
