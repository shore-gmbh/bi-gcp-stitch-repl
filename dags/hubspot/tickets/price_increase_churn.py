...
dag = DAG(
    'hubspot_price_increase_churn',
    default_args=default_args,
    description='DAG to import price increase churn data from hubspot',
...
dataset_id = 'hubspot'
table_name = 'price_increase_churn'
...
def fetch_and_process_tickets():
    ...
    url = f"https://api.hubapi.com/crm/v3/objects/tickets?limit={batch_limit}&properties=organization_id__b_m_&properties=hs_ticket_category&properties=source_type&properties=hubspot_team_id&properties=hs_pipeline_stage&properties=hs_pipeline&properties=closed_date&properties=churn_request_between_price_increase_communication_and_renewal&properties=subject&associations=companies&archived=false"
...
def process_churn_tickets(tickets):
    ...
    tickets.rename(columns = {'associations.companies.results': 'company_id'
                            , 'updatedAt': 'updated_at'
                            , 'createdAt': 'created_at'
                            , 'properties.churn_request_between_price_increase_communication_and_renewal': 'churn_request_between_price_increase_communication_and_renewal'
                            , 'properties.closed_date': 'closed_date'
                            , 'properties.hs_lastmodifieddate': 'hs_lastmodifieddate'
                            , 'properties.hs_pipeline': 'hs_pipeline'
                            , 'properties.hs_pipeline_stage': 'ticket_status'
                            , 'properties.hs_ticket_category': 'hs_ticket_category'
                            , 'properties.hubspot_team_id': 'hubspot_team_id'
                            , 'properties.organization_id__b_m_': 'organization_id__b_m_'
                            , 'properties.source_type': 'source_type'
                            , 'properties.subject': 'subject'
                            ,'createdat': 'created_at'}, inplace=True)

    selected_columns = ['id', 'subject', 'created_at', 'updated_at', 'hs_lastmodifieddate', 'hs_ticket_category', 'source_type', 'hubspot_team_id', 'ticket_status', 'hs_pipeline', 'closed_date', 'churn_request_between_price_increase_communication_and_renewal', 'organization_id__b_m_', 'company_id']
