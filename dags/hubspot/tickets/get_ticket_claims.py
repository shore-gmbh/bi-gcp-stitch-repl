...
dag = DAG(
    'hubspot_tickets_claims',
    default_args=default_args,
    description='DAG to import ticket claims data from hubspot',
...
def fetch_tickets():
    ...
    url = f"https://api.hubapi.com/crm/v3/objects/tickets?associations=companies&properties=,claim,currency,recovered_amount_2,stripe_invoice_id,payment_date,dispute_date&limit={batch_limit}&archived=false"
    ...
    while response.status_code == 200:
        ...
        for obj in tickets_batch:
            data_2 = {
                "id": obj["id"],
                "currency": obj["properties"]["currency"],
                "claim": obj["properties"]["claim"],
                "recovered_amount_2": obj["properties"]["recovered_amount_2"],
                "createdAt": obj["createdAt"],
                "updatedAt": obj["updatedAt"],
                "stripe_invoice_id":obj["properties"]["stripe_invoice_id"],
                "payment_date":obj["properties"]["payment_date"],
                "dispute_date":obj["properties"]["dispute_date"]
            }
...
def save_tickets_data(**kwargs):
    ...
    ti = kwargs['ti']
    df = ti.xcom_pull(task_ids='fetch_tickets')
    #logging.info(df)
...
fetch_tickets_task = PythonOperator(
    task_id='fetch_tickets',
    python_callable=fetch_tickets,
    dag=dag,
)

save_tickets_data_task = PythonOperator(
...
insert_tickets_into_dwh_task = PythonOperator(
...
fetch_tickets_task >> save_tickets_data_task >> insert_tickets_into_dwh_task