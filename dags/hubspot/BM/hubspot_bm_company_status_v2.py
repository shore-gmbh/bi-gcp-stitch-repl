...
def process_organizations(ti):
    ...
    for row in query_results:
        ...
        org_status = row.get('backoffice_bm_organization_status_2')  
        subscription_package = row.get('backoffice_bm_subscription_package_current_plan_2') 

        if company_id is not None:
            company_id = math.trunc(company_id)
            # Update the dictionary to include new fields
            properties = {
                'backoffice_bm_organization_status_2': org_status,  
                'backoffice_bm_subscription_package_current_plan_2': subscription_package
            }
...
def query_bigquery(ti):
    ...
    query = """
                   ...
                   SELECT distinct(company_id), 
                        bi_organization_status as backoffice_bm_organization_status_2,
                        bi_subscription_package as backoffice_bm_subscription_package_current_plan_2
                    FROM dbt_integrations.hubspot_bm_parameter_update_v2  hpppu 
...
with DAG(
    'hubpost_bm_dwh_company_sync_2',
    default_args=default_args,
    description='Hubspot BM DWH sync',
