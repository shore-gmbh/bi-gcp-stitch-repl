...
def process_batches(ti):
    ...
    for batch in batches:
        try:
            ...
            for batch in batches:
                response = update_companies_per_batch(api_key, batch)
...
def update_companies_per_batch(api_key, batch):
    url = f'https://api.hubapi.com/crm/v3/objects/companies/batch/update'
...
def process_organizations(ti):
    ...
    for row in query_results:
        # Assuming 'company_id' and 'field' are keys in your row dictionary
        # You'll need to adjust 'field' to the actual field name you're interested in
        company_id = row.get('company_id')
        field_value = row.get('backoffice_organization_status__b_m__3__updated_from_bi____cloned_')  # Example field
        
        if company_id is not None and field_value is not None:
            company_id = math.trunc(company_id)
            organizations = create_or_update_property(organizations, company_id, {'backoffice_organization_status__b_m__3__updated_from_bi____cloned_': field_value})
    
    # Process 'organizations' as needed, e.g., logging, saving to a file, etc.
    print(organizations)
...
def query_bigquery(ti):
    ...
    query = """
        SELECT company_id,
        is_active_org as backoffice_organization_status__b_m__3__updated_from_bi____cloned_
        FROM `bi-data-replication-gcs.backup.active_orgs_v2`
        WHERE DATE(MONTH) = LAST_DAY(CURRENT_DATE) AND company_id IS NOT NULL
    """
...
with DAG(
    'hubpost_bm_dwh_contact_usage_data',
    default_args=default_args,
    description='Hubspot BM DWH sync active orgs',
