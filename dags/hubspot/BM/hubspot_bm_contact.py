...
def process_organizations(ti):
    ...
    for row in query_results:
        ...
        bookings_created = row.get('bookings_created__total_b_m')  
        sma_created = row.get('sma_created__total_b_m') 
        ssu_no = row.get('ssu_no_oba') 
        ssu_no_sma = row.get('ssu_no_sma') 
        ssu_no_customer = row.get('ssu_no_customer') 
        ssu_no_employees = row.get('ssu_no_employees')
        ssu_no_services = row.get('ssu_no_services')
        backoffice_bm = row.get('backoffice_bm_whitelabel')
#'bookings_created__total_b_m', 'sma_created__total_b_m', 'ssu_no_oba', 'ssu_no_sma', 'ssu_no_customer', 'ssu_no_employees', 'ssu_no_services',
               #  'backoffice_bm_whitelabel', 
        if company_id is not None:
            company_id = math.trunc(company_id)
            # Update the dictionary to include new fields
            properties = {
                'bookings_created__total_b_m': bookings_created,  
                'sma_created__total_b_m': sma_created,
                'ssu_no_oba': ssu_no,
                'ssu_no_sma': ssu_no_sma,
                'ssu_no_customer': ssu_no_customer,
                'ssu_no_employees': ssu_no_employees,
                'ssu_no_services' : ssu_no_services,
                'backoffice_bm' : backoffice_bm
            }
...
def query_bigquery(ti):
    ...
    query = """
            select * from dbt_integrations.hubspot_bm_parameter_update_v2 where vid is not null
    """
...
with DAG(
    'hubpost_bm_dwh_usage_data',
    default_args=default_args,
    description='Hubspot BM DWH contact usage data',
