...
def process_organizations(ti):
    ...
    for row in query_results:
        ...
        org_status = row.get('backoffice_pos_customer_subscription_status_2')  
        subscription_package = row.get('backoffice_pos_subscription_package_current_plan_2') 

        if company_id is not None:
            company_id = math.trunc(company_id)
            # Update the dictionary to include new fields
            properties = {
                'backoffice_pos_customer_subscription_status_2': org_status,  
                'backoffice_pos_subscription_package_current_plan_2': subscription_package
            }
...
def query_bigquery(ti):
    ...
    query = """
                   select distinct (company_id) from dbt_integrations.hubspot_pos_parameter_update hbpu where company_id  IS NOT NULL 
    """
    ...
    # Set the location to the correct region, e.g., 'europe-west3' for Frankfurt
    job_config.location = "europe-west3"
...
with DAG(
    'hubpost_pos_company_sync_V2',
    default_args=default_args,
    description='Hubspot pos company sync',
